...
#     keyboard.add(KeyboardButton(key))
#
# @dp.message_handler(commands=['start', 'help'])
# async def send_welcome(message: Message):
#     await message.reply("Привет! Выбери класс из списка ниже, и я пришлю ссылку.", reply_markup=keyboard)
#
# @dp.message_handler()
# async def handle_message(message: Message):
#     user_input = message.text.strip()
#     logging.info(f"Пользователь {message.from_user.id} отправил сообщение: {user_input}")
#
#     # Отправляем сообщение в группу
#     await bot.send_message(GROUP_ID, f"Сообщение от {message.from_user.full_name} (@{message.from_user.username}):\n{user_input}")
#
#     # Отправляем ссылку, если сообщение совпадает с клавиатурой
#     if user_input in links:
#         await message.reply(f"Вот ссылка для {user_input}: {links[user_input]}")
#     else:
#         await message.reply("Извините, я не нашел ссылку для этого класса. Попробуйте еще раз.")
#
# if name == "main":
